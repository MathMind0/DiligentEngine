...
MTLPixelFormat   TexFormatToMtlPixelFormat(TEXTURE_FORMAT TexFmt);
TEXTURE_FORMAT   MtlPixelFormatToTexFormat(MTLPixelFormat PixelFmt) noexcept;
MTLVertexFormat  InputLayoutFormatToMtlVertexFormat(VALUE_TYPE ValType, Uint32 NumComponents, Bool bIsNormalized);
MTLVertexFormat  ValueTypeToMtlVertexFormat(VALUE_TYPE ValType, Uint32 NumComponents, Bool bIsNormalized) noexcept;
...
MTLBlendOperation BlendOperationToMtlBlendOperation(BLEND_OPERATION BlendOp) noexcept;
...
MTLSamplerMinMagFilter FilterTypeToMtlFilter(FILTER_TYPE FilterType) noexcept;
